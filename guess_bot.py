...
публичный - без шифрования любой мог бы открыть файл на GitHub
...
def message_matches_answer(text, answer):
    ...
    words = re.findall(r"[а-яёa-z0-9]+", text_norm)
...
def handle_private_message(state, user_id, user, text_raw, photo):
    ...
    if text_cmd == "/загадать":
        if state["active_riddle"]:
            ...
            send_message(user_id, "Пришли мне фото крупным планом")
        return True
